...
namespace PotSemestralkaJakubBlunar
{
    ...
    public class StateGamePlay : GameState
    {
        ...
        {
            Console.Clear();
            var actual = Game.Player.ActualRoom;
                ...
                if (command != null)
                {
                    ...
                    switch (split[0])
                    {
                        case "go":
                            ...
                            if (parsed)
                                Game.Loader.Player.ActualRoom =
                                    Game.Loader.Rooms.Find(x => x.Name == Game.Player.NameOfActualRoom);
                            break;
                        case "look":
                            if (split.Length == 1)
                            {
                            ...
                            {
                                var o = Game.Player.Look(split[1]);
                                if (o != null)
                                    if ((o.Type == GameObjectType.Chest) || (o.Type == GameObjectType.Bookshelf))
                                    {
                                        var state = (StateLookAtObject) Game.Manager.GetGameState("lookAtObject");
                                        state.GameObject = o;
